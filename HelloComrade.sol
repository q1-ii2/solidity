...
contract HelloComrades{
    ...
    */
    ...
    ///@param content:  当前同志们说的内容
    ///return   当前的调用状态,true为成功
    function comfortRes(string calldata content)external payable notLeader returns(bool){
       if(step!=3){
            revert(UNKNOWN);
        }
    ...
    */
    ...
    ///@notice  用于领导对合约进行销毁
    ///@dev     只能在dev为4的时候调用,只能领导调用
    ///return   当前的调用状态,true为成功
     function distruct()external onlyLeader returns(bool){
       if(step!=4){
            revert(UNKNOWN);
        }
        step=4;
        emit Log("destruct",msg.sender,address(this).balance,msg.data);
        selfdestruct(payable(msg.sender)); //自毁
        return true;
    }
}
